...
BEGIN
    SELECT PJ.id_pelanggan INTO pelanggan FROM TR_PEMESANAN_STATUS PS
    JOIN TR_PEMESANAN_JASA PJ ON (PS.id_tr_pemesanan = PJ.id) 
    WHERE PS.id_tr_pemesanan = NEW.id_tr_pemesanan;

    SELECT PJ.id_pekerja INTO pekerja FROM TR_PEMESANAN_STATUS PS
    JOIN TR_PEMESANAN_JASA PJ ON (PS.id_tr_pemesanan = PJ.id) 
    WHERE PS.id_tr_pemesanan = NEW.id_tr_pemesanan;

    SELECT PJ.total_biaya INTO biaya FROM TR_PEMESANAN_STATUS PS
    JOIN TR_PEMESANAN_JASA PJ ON (PS.id_tr_pemesanan = PJ.id) 
    WHERE PS.id_tr_pemesanan = NEW.id_tr_pemesanan;
